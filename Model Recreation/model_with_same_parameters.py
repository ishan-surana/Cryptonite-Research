...
from tensorflow.keras.layers import Embedding, Conv1D, MaxPooling1D, Dense, Input, Concatenate, Dropout, Flatten, BatchNormalization
...
X_pad = pad_sequences(X_seq, maxlen=93)
...
input_content = Input(shape=(93,), name='content_input')
input_text_structure = Input(shape=(16,), name='text_structure_input')
input_url_structure = Input(shape=(1000,), name='url_structure_input')  # Adjusted size to match the provided vector size

# Convolutional layer with L2 regularization and Batch Normalization
l2_lambda = 0.01
embedding = Embedding(input_dim=len(tokenizer.word_index)+1, output_dim=93, input_length=93)(input_content)
conv_layer = Conv1D(8, 8, activation='relu', kernel_regularizer=regularizers.l2(l2_lambda))(embedding)
...
batch_norm_conv = BatchNormalization()(flattened_layer)

# GlobalMaxPooling1D layer to match the specified size of the flattened vector
# global_pooling_layer = GlobalMaxPooling1D()(pooling_layer)

# Concatenate all input layers
concatenated_inputs = Concatenate()([batch_norm_conv, input_text_structure, input_url_structure])  # Adjusted size to match the provided vector size

# Fully connected layers with L2 regularization and Batch Normalization
dense1 = Dense(155, activation='relu', kernel_regularizer=regularizers.l2(l2_lambda))(concatenated_inputs)
batch_norm_dense1 = BatchNormalization()(dense1)

dense2 = Dense(105, activation='relu', kernel_regularizer=regularizers.l2(l2_lambda))(batch_norm_dense1)
batch_norm_dense2 = BatchNormalization()(dense2)

dense3 = Dense(50, activation='relu', kernel_regularizer=regularizers.l2(l2_lambda))(batch_norm_dense2)
batch_norm_dense3 = BatchNormalization()(dense3)

dense4 = Dense(25, activation='relu', kernel_regularizer=regularizers.l2(l2_lambda))(batch_norm_dense3)
batch_norm_dense4 = BatchNormalization()(dense4)

dense5 = Dense(25, activation='relu', kernel_regularizer=regularizers.l2(l2_lambda))(batch_norm_dense4)
batch_norm_dense5 = BatchNormalization()(dense5)

dense6 = Dense(10, activation='relu', kernel_regularizer=regularizers.l2(l2_lambda))(batch_norm_dense5)
batch_norm_dense6 = BatchNormalization()(dense6)

# Output layer with softmax activation
output = Dense(5, activation='softmax')(batch_norm_dense6)  # Adjusted output neurons to 5

# Define the model
model = Model(inputs=[input_content, input_text_structure, input_url_structure], outputs=output)

# Compile the model with Adam optimizer and Kullback-Leibler divergence loss function
