...
# Process URL features
train_urls = process_url_features(data.loc[X_train.index, 'urls'])
test_urls = process_url_features(data.loc[X_test.index, 'urls'])

# Convert URLs to numerical values
num_train_urls = np.array([len(u) for u in train_urls])
num_test_urls = np.array([len(u) for u in test_urls])
